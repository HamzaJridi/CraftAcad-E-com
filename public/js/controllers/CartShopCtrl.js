 ...
 * add, delete products*/
angular.module('myApp').controller('CartShopCtrl', ['$location', '$scope', '$http', '$routeParams', '$rootScope',
    ...
    $scope.disableDate = function (date) {
      ...
      if ($scope.product.listReser) {
        for (var i = 0; i < $scope.product.listReser.length; i++) {
          if ((new Date($scope.product.listReser[i].dateRes).getFullYear() === date.getFullYear()) && (new Date($scope.product.listReser[i].dateRes).getMonth() === date.getMonth()) && (new Date($scope.product.listReser[i].dateRes).getDate() === date.getDate())) {
            available = false;
          }
        }
      }
